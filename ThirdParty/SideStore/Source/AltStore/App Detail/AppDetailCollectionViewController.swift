...
private extension AppDetailCollectionViewController
{
    ...
    {
        let alertController = UIAlertController(title: NSLocalizedString("Other Entitlements", comment: ""), message: NSLocalizedString("KittyStore does not have detailed information for these entitlements.", comment: ""), preferredStyle: .alert)
        alertController.addAction(.ok)
        self.present(alertController, animated: true)
    }
}
