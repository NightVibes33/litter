...
pub extern "C" fn rust_bridge_instproxy_install_with_bundle_id(
    ...
    let mut client_opts = Plist::new_dict();
    client_opts
        .dict_insert_item("CFBundleIdentifier", Plist::new_string(bundle_id))
        .unwrap();
...
pub extern "C" fn rust_bridge_debug_app_post17(
    ...
    let app_id = unsafe { CStr::from_ptr(app_id) }
        ...
        .unwrap()
        .to_string();
    let muxer_addr = unsafe { CStr::from_ptr(muxer_addr) }
        .to_str()
        .unwrap()
        .to_string();
    let device_ip = unsafe { CStr::from_ptr(device_ip) }
        .to_str()
        .unwrap()
        .to_string();
