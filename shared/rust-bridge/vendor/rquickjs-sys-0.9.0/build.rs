...
fn main() {
    ...
    if target_os == "ios" {
        let target = env::var("TARGET").expect("TARGET is required for iOS bindgen");
        ...
        let output = process::Command::new("xcrun")
            ...
            .output()
            .expect("failed to locate the Apple SDK with xcrun");
        if !output.status.success() {
            panic!(
        ...
        let sdk_path = String::from_utf8(output.stdout)
            ...
            .trim()
            .to_string();
        bindgen_cflags.extend(["-isysroot".to_string(), sdk_path]);
    }
