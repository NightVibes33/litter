...
- (instancetype)initWithItems:(NSDictionary*)items withKernelSurfaceProcess:(ksurface_proc_t*)proc withSession:(NXWindowSessionApplication*)session
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        id sharedApplication = [NSClassFromString(@"UIApplication") performSelector:NSSelectorFromString(@"sharedApplication")];
        if([sharedApplication respondsToSelector:NSSelectorFromString(@"statusBarOrientation")])
        {
            NSNumber *statusBarOrientation = [sharedApplication valueForKey:@"statusBarOrientation"];
            if([statusBarOrientation respondsToSelector:@selector(integerValue)])
            {
                interfaceOrientation = (UIInterfaceOrientation)statusBarOrientation.integerValue;
            }
        }
