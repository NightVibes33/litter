...
The in-app KittyStore tab follows SideStore's five-tab shape: News, Sources, Browse, My Apps, and Settings. News is a store feed, Sources owns the saved SideStore/AltStore source list, Browse aggregates apps from every loaded source instead of being tied to Litter, My Apps is reserved for installed-device actions through minimuxer, and Settings owns account, certificate, pairing, LocalDevVPN, and diagnostics setup. Its Sign screen follows Feather's signer layout: IPA customization, certificate/provisioning selection, advanced Modify rows, Entitlements, Tweaks, Properties, and Start Signing.

KittyStore Settings owns the SideStore `.sideconf` account import, Apple ID login, Anisette server selection, `.p12` certificate import, certificate password validation, pairing-file import, and LocalDevVPN launcher. Pairing files and per-app provisioning profiles are staged into app storage and restored on the next KittyStore load, so importing them is not just a one-session UI event. The `.sideconf` path preserves the Apple ID, password, signing certificate, certificate password, `local_user`, and `adiPB` fields while validating the certificate before saving it. BuildKit Settings is intentionally read-only for this state now, so SideStore and Feather options are not scattered outside KittyStore.
