...
On-device models now have a native llama.cpp token-generation bridge wired into the Swift runtime when `apps/ios/Frameworks/llama.xcframework` is present, plus guarded fakefs tools, approval request state for shell/write actions, retry events, streaming tool-call state, device-derived context defaults, and accurate streamed download progress with cancel support, automatic post-install smoke validation, manual Verify Model actions, and native cancellation hooks. Installed verified models can open a Local Agent workspace with context file selection, streaming generation, approval-gated shell/write tools, diff previews, retries, cancellation, partial-output recovery, and the selected model's runtime settings.

TurboQuant is modeled as an experimental llama.cpp runtime capability, not a fake always-on switch. The native bridge now passes advanced llama.cpp runtime settings through to the engine, including Metal GPU layer selection, CPU fallback, thread count, top-p, top-k, repeat penalty, flash-attention-enabled contexts, and KV cache type (`F16`, `Q8`, `Q4`, or TurboQuant modes when the linked ggml runtime reports them). Standard builds report TurboQuant unavailable and keep TurboQuant KV cache modes disabled. A build that links a TurboQuant-capable llama.cpp fork can report supported TurboQuant types through the native bridge, at which point the settings UI enables TurboQuant 3-bit/4-bit KV cache choices instead of silently faking support.
