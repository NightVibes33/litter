...
TurboQuant is modeled as an experimental llama.cpp runtime capability, not a fake always-on switch. Standard builds report TurboQuant unavailable and keep TurboQuant KV cache modes disabled. A future build that links a TurboQuant-capable llama.cpp fork can report supported modes through the native bridge, at which point the settings UI can enable TurboQuant 3-bit/4-bit KV cache choices.
